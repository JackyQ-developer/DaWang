jQuery(document).ready(function($) {
	...
	$('#register').click(function(){
		...
		$.ajax({
			url: domain + '/api/user.info/user_mobile',
			type: 'POST',
			data: {
				identifier: $('#mobile').val(),
			...
			success: function(response){
				if(response == 1){
					mui.toast('验证成功');
					plus.webview.open('../changeMobile2/changeMobile2.html','changeMobile2',{},"slide-in-right");
					plus.webview.close(plus.webview.currentWebview(),'none');
				}else if(response == 2){
