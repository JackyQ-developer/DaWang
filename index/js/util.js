var util = {
	options: {
		ACTIVE_COLOR: "#ff5a54",
		NORMAL_COLOR: "#000",
		subpages: ["../nearby/nearby.html","../chatList/chatList.html", "../personalCenter/personalCenter.html"]
	},
	/**
	 *  简单封装了绘制原生view控件的方法
	...
	changeSubpage: function(targetPage, activePage, aniShow) {
		...
		if(mui.os.ios || aniShow[targetPage]) {
			...
			//否则，使用zoom-out动画，且保存变量
			var temp = {};
			temp[targetPage] = "true";
			mui.extend(aniShow, temp);
			plus.webview.show(targetPage, "zoom-out", 1200);
		}
		//隐藏当前 除了第一个父窗口
	 ...
	 * 绘制消息小红点
	 */
	drawSuperIcon: function(currIndex, color) {
		var self = plus.webview.currentWebview();
		/**
		 * 画一个角标
		 */
		var superscript = this.drawNative('superscript', {
			bottom: '35px',
			left: '70%',
			width: '10px',
			height: '10px',
		...
		// append 到父webview中
		self.append(superscript);
	},
	/*
	 * 关闭消息小红点
	 */
	closeSuperIcon: function(currIndex, color) {
		var self = plus.webview.currentWebview();
		/**
		 * 画一个角标
		 */
		var superscript =  plus.nativeObj.View.getViewById('superscript');
		// 关闭
		if(superscript != null){
			superscript.close();
		}
