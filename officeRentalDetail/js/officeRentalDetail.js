...
mui.plusReady(function(){
	...
	function getData(){
		...
		$.ajax({
			url: domain + '/api/house.show/officeRentPcs',
			type: 'POST',
			data: {
				houseId: self.pid,
				token: token
	...
	var app = new Vue({
		...
		computed:{
			genreValue:function(){
				return getBuildingGenreByChinese(this.genre);
			},
			priceType:function(){
				return getPriceTypeByChinese(this.price_type);
			}
		},
		mounted: getData,
		updated: function () {
	...
	mui('.recommends').on('tap','.recommend',function(){
		...
		mui.openWindow({
			url:'../officeRentalDetail/officeRentalDetail.html',
			extras: {
				pid: ids
			},
			waiting: {
