...
mui.init({
	...
	preloadPages: [				//预加载
		{
	      url:"../houseSelect/houseSelect.html",
	      id: "houseSelect",
	      styles:{
	      	left: '20%',
	      	right: '0',
...
$('#search').focus(function(){
	...
	moreSwitch = false;
	plus.webview.hide( 'houseSelect', 'slide-out-right' );
	mask.close();
})
...
mui.plusReady(function(){
	...
	document.getElementById('more').addEventListener('tap', function (e) {
		...
		plus.webview.show( "houseSelect", "slide-in-right" );
		moreSwitch = true;
	});
});
...
function pulldownRefresh() {
	...
	$.ajax({
		url: domain + '/api/house.show/secondRefresh',
		type: 'POST',
...
function pullupRefresh() {
	...
	searchCondition2.num = app.num;
	$.ajax({
		url: domain + '/api/house.show/secondList',
		type: 'POST',
		data: searchCondition2,
		timeout: 10000,
		success: function(response){
	...
	$.ajax({
		...
		success: function(response){
			...
			if(app.showData.length<=5){
				...
				mui('#pullrefresh').pullRefresh().refresh(true);		//大于5条重置上拉加载
			}
		},
		error: function(xhr,textStatus){
			if(textStatus=='timeout'){
...
$('.input-container input').bind('input',function (e) {
	...
	if($(this).siblings('input').val() != ''){
		...
		searchShow.price = firstChild+'-'+lastChild+'万';
	}
});
